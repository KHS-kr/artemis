...
class VisualStepSummarizer(StepMemoryService):
    ...
    def __init__(
        ...
        *,
        ...
        # Initialize the lightweight VLM from the configured summarizer node.
        # `model_name` (agent.flash.step_summarizer.model) is a model-name
        # override, not a provider switch: it holds a Gemini name, so it is
        # applied only when that node resolves to Google. Building a Google
        # client because a name was configured used to abort every run on any
        # other provider, before the first step.
        self._model_name = model_name or "gemini-2.5-flash-lite"
        try:
            self._llm = get_lens_llm(ctx, "summarizer", model_override=model_name)
        except Exception as exc:
            # Fall back to the node as configured - a different call from the
            # one that just failed, unlike the historical retry of the same
            # Google construction.
            logger.warning(
                "Step summarizer model %r unavailable (%s); using the configured "
                "summarizer node instead.",
                model_name,
                exc,
            )
            self._llm = get_lens_llm(ctx, "summarizer")
        try:
    ...
    def _meter_lens_call(self, response: Any) -> None:
        ...
        ``get_lens_llm`` model needs explicit metering here. Lens prompts
